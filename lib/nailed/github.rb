  ...
  class Github
    ...
    def get_pull_requests(state: 'open')
      ...
      Nailed::Config.all_repositories['github'].each do |repo|
        ...
        Changerequest.select(:change_number, :state, :rname).where(state: "open", rname: repo.name, origin: "github").each do |pr|
    ...
    def write_pulltrends(org, repo)
      ...
      open = Changerequest.where(rname: repo, state: "open", origin: "github").count
      closed = Changerequest.where(rname: repo, state: "closed", origin: "github").count
    ...
    def write_allpulltrends
      ...
      open = Changerequest.where(state: "open", origin: "github").count
      closed = Changerequest.where(state: "closed", origin: "github").count
      attributes = { time: Time.new.strftime("%Y-%m-%d %H:%M:%S"),
                     open: open,
                     closed: closed,
                     origin: "github" }
